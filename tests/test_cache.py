...
def test_cache_velocity_aggregation():
    ...
    # Add transactions in chronological order (oldest first)
    cache.add_transaction("user1@ybl", "dev_1", 500.0, timestamp=now - 80) # Outside 60s window
    ...
    # Check 1m velocity (should include the 3 transactions in the last 10, 5, and 2 seconds)
    count, total = cache.get_vpa_velocity("user1@ybl", 60, current_time=now)
    assert count == 3
    assert total == 450.0
    
    # Check 10m velocity (should include the 80s old transaction as well)
...
if __name__ == "__main__":
    ...
    print("[PASS] All VelocityCache tests passed!")
